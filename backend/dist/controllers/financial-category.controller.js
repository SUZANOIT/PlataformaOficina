...
exports.FinancialCategoryController = void 0;
...
class FinancialCategoryController {
    ...
    static async list(req, res) {
        try {
            ...
            let categories = await prisma_1.prisma.financialCategory.findMany({
                where: { companyId },
                orderBy: { name: 'asc' },
            });
            ...
            if (categories.length === 0) {
                ...
                await prisma_1.prisma.financialCategory.createMany({
                    data: defaultCategories,
                });
                // Buscar novamente após seeding
                categories = await prisma_1.prisma.financialCategory.findMany({
                    where: { companyId },
                    orderBy: { name: 'asc' },
                });
            }
    ...
    static async create(req, res) {
        try {
            ...
            const exists = await prisma_1.prisma.financialCategory.findFirst({
                where: {
                    companyId,
            ...
            const newCategory = await prisma_1.prisma.financialCategory.create({
                data: {
                    name,
                    type,
                    companyId,
    ...
    static async update(req, res) {
        try {
            ...
            const category = await prisma_1.prisma.financialCategory.findFirst({
                where: { id, companyId },
            });
            if (!category) {
            ...
            const duplicate = await prisma_1.prisma.financialCategory.findFirst({
                where: {
                    companyId,
                    name: { equals: name, mode: 'insensitive' },
                    type,
            ...
            const updated = await prisma_1.prisma.financialCategory.update({
                where: { id },
                data: { name, type },
            });
    ...
    static async delete(req, res) {
        try {
            ...
            await prisma_1.prisma.financialCategory.delete({
                where: { id },
            });
...
exports.FinancialCategoryController = FinancialCategoryController;
