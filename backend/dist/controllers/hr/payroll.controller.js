    ...
    async getEstimate(req, res) {
        try {
            ...
            if (!employee.salaryConfig)
                return res.status(400).json({ error: 'Configuração salarial ausente' });
            ...
            employee.towingQuotes.forEach(quote => {
                ...
                if (cfg.valorAtendimento)
                    comissaoTotal += cfg.valorAtendimento;
                if (cfg.valorKm && quote.distanciaKm)
                    comissaoTotal += (cfg.valorKm * quote.distanciaKm);
                if (cfg.comissaoPercentual)
                    comissaoTotal += (quote.valorTotal * (cfg.comissaoPercentual / 100));
            });
