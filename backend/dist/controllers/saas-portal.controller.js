...
exports.SaaSPortalController = {
    ...
    async listTenants(req, res) {
        try {
            ...
            const companyIds = tenants.map(t => t.companyId).filter(Boolean);
            ...
            const workshopCounts = await prisma_1.prisma.oficina.groupBy({
                by: ['companyId'],
                _count: { id: true },
                where: { companyId: { in: companyIds } }
            });
            const osCounts = await prisma_1.prisma.quote.groupBy({
                by: ['companyId'],
                _count: { id: true },
                where: { companyId: { in: companyIds } }
            });
            const countMap = (arr) => arr.reduce((acc, curr) => {
                if (curr.companyId)
                    acc[curr.companyId] = curr._count.id;
                return acc;
            }, {});
