...
exports.AdminSaaSController = void 0;
const prisma_1 = require("../lib/prisma");
const bcrypt_1 = __importDefault(require("bcrypt"));
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
// Helper for administrative audit logging
async function registrarAuditoria(usuario, action, modulo, registroAfetado, tenant, ip) {
    try {
        await prisma_1.prisma.saaSAuditoria.create({
            data: {
                usuario,
                acao: action,
                modulo,
...
async function inicializarSaaSSeVazio() {
    try {
        const planosCount = await prisma_1.prisma.saaSPlano.count();
        if (planosCount === 0) {
            // Create default plans
            await prisma_1.prisma.saaSPlano.createMany({
                data: [
        ...
        const configCount = await prisma_1.prisma.saaSConfiguracao.count();
        if (configCount === 0) {
            await prisma_1.prisma.saaSConfiguracao.createMany({
                data: [
        ...
        const empresasCount = await prisma_1.prisma.saaSEmpresa.count();
        if (empresasCount === 0) {
            ...
            const defaultPlan = await prisma_1.prisma.saaSPlano.findFirst({ where: { nome: 'Enterprise' } });
            for (const comp of companies) {
                await prisma_1.prisma.saaSEmpresa.create({
                    data: {
                        id: comp.id,
                ...
                if (defaultPlan) {
                    await prisma_1.prisma.saaSAssinatura.create({
                        data: {
                            empresaId: comp.id,
                    ...
                    await prisma_1.prisma.saaSFaturamento.create({
                        data: {
                            empresaId: comp.id,
                            planoId: defaultPlan.id,
                            valor: defaultPlan.valorMensal,
...
exports.AdminSaaSController = {
    ...
    getDashboardStats: async (req, res) => {
        ...
        try {
            const activeCompanies = await prisma_1.prisma.saaSEmpresa.count({ where: { status: 'ATIVO' } });
            const suspendedCompanies = await prisma_1.prisma.saaSEmpresa.count({ where: { status: 'SUSPENSO' } });
            const activeSubs = await prisma_1.prisma.saaSAssinatura.count({ where: { status: 'ATIVA' } });
            // Calculate MRR & ARR from active subscriptions
            const subscriptions = await prisma_1.prisma.saaSAssinatura.findMany({ where: { status: 'ATIVA' } });
            ...
            const totalOS = await prisma_1.prisma.quote.count({ where: { status: { not: 'Orçamento' } } });
            const totalQuotes = await prisma_1.prisma.quote.count({ where: { status: 'Orçamento' } });
            // Monthly revenue from billing table
            const billing = await prisma_1.prisma.saaSFaturamento.findMany({
                where: {
                    status: 'PAGO'
                }
            });
            ...
            const subscriptionsByPlan = await prisma_1.prisma.saaSAssinatura.groupBy({
                by: ['planoId'],
                _count: { id: true }
            });
            const plans = await prisma_1.prisma.saaSPlano.findMany();
            ...
            const companiesByStatusGroup = await prisma_1.prisma.saaSEmpresa.groupBy({
                by: ['status'],
                _count: { id: true }
            });
    ...
    listEmpresas: async (req, res) => {
        await inicializarSaaSSeVazio();
        try {
            const empresas = await prisma_1.prisma.saaSEmpresa.findMany({
                orderBy: { createdAt: 'desc' }
            });
            const planos = await prisma_1.prisma.saaSPlano.findMany();
    ...
    getEmpresa: async (req, res) => {
        ...
        try {
            const empresa = await prisma_1.prisma.saaSEmpresa.findUnique({
                where: { id }
            });
            if (!empresa) {
    ...
    createEmpresa: async (req, res) => {
        ...
        try {
            ...
            const saasEmpresa = await prisma_1.prisma.saaSEmpresa.create({
                data: {
                    razaoSocial,
                    nomeFantasia,
                    cnpj,
            ...
            if (planoId) {
                const plan = await prisma_1.prisma.saaSPlano.findUnique({ where: { id: planoId } });
                if (plan) {
                    await prisma_1.prisma.saaSAssinatura.create({
                        data: {
    ...
    updateEmpresa: async (req, res) => {
        ...
        try {
            ...
            const updated = await prisma_1.prisma.saaSEmpresa.update({
                where: { id },
                data: {
                    razaoSocial,
                    nomeFantasia,
    ...
    deleteEmpresa: async (req, res) => {
        ...
        const id = req.params.id;
        try {
            const deleted = await prisma_1.prisma.saaSEmpresa.update({
                where: { id },
                data: { status: 'INATIVO' }
            });
    ...
    suspenderEmpresa: async (req, res) => {
        const userEmail = req.userEmail || 'admin';
        const { id } = req.body;
        try {
            const updated = await prisma_1.prisma.saaSEmpresa.update({
    ...
    acessarTenant: async (req, res) => {
        const userEmail = req.userEmail || 'admin';
        const { id } = req.body; // SaaSEmpresa ID
        try {
            const saasEmpresa = await prisma_1.prisma.saaSEmpresa.findUnique({ where: { id } });
            ...
            }, process.env.JWT_SECRET || 'fallback-secret-key-mca-oficina-saas-platform-token-sig', { expiresIn: '1h' });
    ...
    getHistoricoEmpresa: async (req, res) => {
        const id = req.params.id;
        try {
            const audits = await prisma_1.prisma.saaSAuditoria.findMany({
                where: { tenant: id },
                orderBy: { createdAt: 'desc' }
            });
    ...
    getPlano: async (req, res) => {
        const id = req.params.id;
        try {
            const plano = await prisma_1.prisma.saaSPlano.findUnique({ where: { id } });
            if (!plano)
    ...
    duplicatePlano: async (req, res) => {
        const userEmail = req.userEmail || 'admin';
        const { id } = req.body;
        try {
            const original = await prisma_1.prisma.saaSPlano.findUnique({ where: { id } });
            if (!original)
                return res.status(404).json({ error: 'Plano original não encontrado' });
            const copy = await prisma_1.prisma.saaSPlano.create({
                data: {
    ...
    // 4. Assinaturas CRUD
    listAssinaturas: async (req, res) => {
        try {
            const assinaturas = await prisma_1.prisma.saaSAssinatura.findMany({
                orderBy: { createdAt: 'desc' }
            });
            const empresas = await prisma_1.prisma.saaSEmpresa.findMany();
            const planos = await prisma_1.prisma.saaSPlano.findMany();
    ...
    renovarAssinatura: async (req, res) => {
        const userEmail = req.userEmail || 'admin';
        const { id } = req.body;
        try {
            const subscription = await prisma_1.prisma.saaSAssinatura.findUnique({ where: { id } });
            if (!subscription)
            ...
            const updated = await prisma_1.prisma.saaSAssinatura.update({
                where: { id },
                data: {
                    status: 'ATIVA',
            ...
            await prisma_1.prisma.saaSEmpresa.update({
    ...
    suspenderAssinatura: async (req, res) => {
        const userEmail = req.userEmail || 'admin';
        const { id } = req.body;
        try {
            const subscription = await prisma_1.prisma.saaSAssinatura.findUnique({ where: { id } });
    ...
    reativarAssinatura: async (req, res) => {
        const userEmail = req.userEmail || 'admin';
        const { id } = req.body;
        try {
            const subscription = await prisma_1.prisma.saaSAssinatura.findUnique({ where: { id } });
    ...
    gerarCobranca: async (req, res) => {
        const userEmail = req.userEmail || 'admin';
        const { empresaId, planoId, valor, competencia } = req.body;
        try {
            const faturamento = await prisma_1.prisma.saaSFaturamento.create({
                data: {
                    empresaId,
                    planoId,
                    valor: Number(valor),
    ...
    redefinirSenhaUsuario: async (req, res) => {
        const userEmail = req.userEmail || 'admin';
    ...
    alterarPerfilUsuario: async (req, res) => {
        const userEmail = req.userEmail || 'admin';
    ...
    getAuditoriaUsuario: async (req, res) => {
        const email = req.params.email;
        try {
            const logs = await prisma_1.prisma.saaSAuditoria.findMany({
                where: { usuario: email },
                orderBy: { createdAt: 'desc' }
            });
    ...
    // 6. Faturamento
    listFaturamentos: async (req, res) => {
        try {
            const faturamentos = await prisma_1.prisma.saaSFaturamento.findMany({
                orderBy: { createdAt: 'desc' }
            });
            const empresas = await prisma_1.prisma.saaSEmpresa.findMany();
            const planos = await prisma_1.prisma.saaSPlano.findMany();
    ...
    alterarStatusFaturamento: async (req, res) => {
        const userEmail = req.userEmail || 'admin';
        const { id, status } = req.body;
        try {
            const updated = await prisma_1.prisma.saaSFaturamento.update({
                where: { id },
                data: {
                    status,
    ...
    getConfiguracoes: async (req, res) => {
        await inicializarSaaSSeVazio();
        try {
            const configs = await prisma_1.prisma.saaSConfiguracao.findMany();
            const result = {};
            configs.forEach(c => {
                try {
    ...
    salvarConfiguracoes: async (req, res) => {
        ...
        try {
            ...
            const config = await prisma_1.prisma.saaSConfiguracao.upsert({
                where: { chave: key },
