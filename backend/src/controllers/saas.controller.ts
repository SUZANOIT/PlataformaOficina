...
const createSaaSCompanySchema = z.object({
...
const updateSaaSCompanySchema = z.object({
  razaoSocial: z.string(),
  nomeFantasia: z.string().nullable().optional(),
  cnpj: z.string(),
  cnpjSemMascara: z.string(),
...
export const SaaSController = {
  ...
  async createCompany(req: Request, res: Response) {
    try {
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        // Hash password
        const hashedPassword = await bcrypt.hash(data.adminSenha, 10);
        ...
        // Module keys map
        const planModulesKeys: Record<string, string[]> = {
  ...
  async buscarCnpj(req: Request, res: Response) {
    try {
      const { cnpj } = req.body;
      if (!cnpj) {
  ...
  async acessarCliente(req: Request, res: Response) {
    try {
