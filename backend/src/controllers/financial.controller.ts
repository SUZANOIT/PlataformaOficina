    ...
    try {
      ...
      if (body.linkedQuotes && body.linkedQuotes.length > 0) {
        for (const link of body.linkedQuotes) {
          let quote: any = await prisma.quote.findUnique({
          ...
          let isTowing = false;
          if (!quote) {
            quote = await prisma.towingQuote.findUnique({
              where: { id: link.quoteId },
              include: {
                linkedPayables: {
                  include: { payable: true }
          ...
          const totalUtilizado = quote.linkedPayables
            .filter((l: any) => l.payable.status !== 'CANCELADA' && l.payable.status !== 'REPROVADA')
            .reduce((sum: number, l: any) => sum + l.valorVinculado, 0);
      ...
      const creatorUser = userId ? await prisma.user.findUnique({
        where: { id: userId },
        select: { name: true }
      }) : null;
      ...
      for (let i = 0; i < installments; i++) {
        ...
        const payable = await prisma.financialPayable.create({
          data: {
            ...
            linkedQuotes: body.linkedQuotes && body.linkedQuotes.length > 0 ? {
              create: body.linkedQuotes.map((l: any) => ({
        ...
        const quoteNumbers = [];
        if (body.linkedQuotes && body.linkedQuotes.length > 0) {
          for (const l of body.linkedQuotes as any[]) {
            if (l.isTowing) {
              const qRecord = await prisma.towingQuote.findUnique({ where: { id: l.quoteId }, select: { numeroOrcamento: true } });
              if (qRecord) quoteNumbers.push(qRecord.numeroOrcamento);
            } else {
              const qRecord = await prisma.quote.findUnique({ where: { id: l.quoteId }, select: { numeroOrcamento: true } });
              if (qRecord) quoteNumbers.push(qRecord.numeroOrcamento);
            }
          }
        }
      ...
      if (updateFields.linkedQuotes !== undefined) {
        if (updateFields.linkedQuotes && updateFields.linkedQuotes.length > 0) {
          for (const link of updateFields.linkedQuotes) {
            ...
            const totalUtilizado = quote.linkedPayables
              .filter((l: any) => l.payable.status !== 'CANCELADA' && l.payable.status !== 'REPROVADA' && l.payableId !== id)
              .reduce((sum: number, l: any) => sum + l.valorVinculado, 0);
      ...
      if (body.linkedQuotes && body.linkedQuotes.length > 0) {
        for (const link of body.linkedQuotes) {
          ...
          const totalUtilizado = quote.linkedReceivables
            .filter((l: any) => l.receivable.status !== 'CANCELADA' && l.receivable.status !== 'REPROVADA')
            .reduce((sum: number, l: any) => sum + l.valorVinculado, 0);
      ...
      if (updateFields.linkedQuotes !== undefined) {
        if (updateFields.linkedQuotes && updateFields.linkedQuotes.length > 0) {
          for (const link of updateFields.linkedQuotes) {
            ...
            const totalUtilizado = quote.linkedReceivables
              .filter((l: any) => l.receivable.status !== 'CANCELADA' && l.receivable.status !== 'REPROVADA' && l.receivableId !== id)
              .reduce((sum: number, l: any) => sum + l.valorVinculado, 0);
        ...
        // Se a validação passou, limpar os links antigos e criar os novos
        await prisma.receivableQuoteLink.deleteMany({
          where: { receivableId: id }
        });

        if (updateFields.linkedQuotes && updateFields.linkedQuotes.length > 0) {
          await prisma.receivableQuoteLink.createMany({
            data: updateFields.linkedQuotes.map((l: any) => ({
              receivableId: id,
              quoteId: l.isTowing ? null : l.quoteId,
              towingQuoteId: l.isTowing ? l.quoteId : null,
