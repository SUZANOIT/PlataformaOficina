            ...
            <table className="w-full border-collapse text-left text-sm table-fixed break-words">
              <thead className="bg-muted/40 border-b border-border text-muted-foreground uppercase text-[10px] tracking-wider font-bold">
                <tr>
                  ...
                  <th className="p-4 w-5/12 md:w-3/12 lg:w-2/12">Origem</th>
                ...
                {receivables.map((r) => (
                  <tr key={r.id} className="hover:bg-secondary/20 transition-colors">
                    ...
                    <td className="p-4 font-medium text-foreground truncate">
                      ...
                      {r.quote && (
                        <div className="text-[10px] text-primary font-bold flex items-center gap-1 mt-0.5 truncate" title="Orçamento vinculado">
                          <span className="truncate">📋 Ref. Orçamento #{r.quote.numeroOrcamento}</span>
                        </div>
                      )}
                    </td>
                    <td className="p-4 text-muted-foreground truncate hidden lg:table-cell">{r.descricao}</td>
                    <td className="p-4 text-right font-extrabold text-foreground truncate">{formatCurrency(r.valor)}</td>
                    <td className="p-4 hidden sm:table-cell truncate">
                      <div className="flex items-center gap-1.5 text-xs font-semibold text-foreground truncate">
                        ...
                        <span className="truncate">{formatDate(r.vencimento)}</span>
                      </div>
                    </td>
                    ...
                    <td className="p-4 hidden xl:table-cell truncate">
                      {r.attachments && r.attachments.length > 0 ? (
                        <div className="flex items-center text-primary truncate" title={`${r.attachments.length} anexo(s)`}>
                          <Paperclip size={14} className="shrink-0" />
                          <span className="text-[10px] font-bold ml-0.5 truncate">{r.attachments.length}</span>
                        </div>
                      ) : (
