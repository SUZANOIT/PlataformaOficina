...
export function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Oficina e Usuário cadastrados com sucesso!');
      navigate('/login');
    } catch (error: any) {
      alert(error.message);
    }
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md bg-card border border-border p-8 rounded-2xl shadow-sm">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full py-2 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 font-medium transition mt-2"
          >
            Cadastrar Oficina e Acessar
          </button>
        </form>
