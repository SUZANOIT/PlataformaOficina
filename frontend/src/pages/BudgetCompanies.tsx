...
export function BudgetCompanies() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isDuplicate = companies.some(
      c => c.cnpjSemMascara === cnpjLimpo && c.id !== selectedCompany?.id
    );
    if (isDuplicate) {
      toast.error('Já existe uma empresa emissora com este CNPJ.');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        handleApiError(response, 'Erro ao salvar dados da empresa.');
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <CompanyModalBreadcrumb isEdit={!!selectedCompany} />
