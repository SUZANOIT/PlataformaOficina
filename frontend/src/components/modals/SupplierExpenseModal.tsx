...
interface SupplierExpenseModalProps {
  isOpen: boolean;
  onClose: () => void;
  supplier: any;
}
...
export function SupplierExpenseModal({ isOpen, onClose, supplier }: SupplierExpenseModalProps) {
  ...
  const fetchDashboard = async () => {
    ...
    try {
      ...
      const response = await api.get(`/registry/suppliers/${supplier.id}/expenses?${params.toString()}`);
  ...
  const renderTrend = (value: number) => {
    if (value > 0) {
      return <span className="text-rose-500 flex items-center text-xs font-bold bg-rose-500/10 px-2 py-0.5 rounded-full"><TrendingUp size={12} className="mr-1"/> +{value.toFixed(1)}%</span>;
    }
    if (value < 0) {
      ...
      <div className="bg-card/95 border border-border/50 w-full max-w-[95vw] h-full max-h-[98vh] rounded-[2rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-300 relative flex flex-col ring-1 ring-white/10">
        ...
        <div className="px-8 py-5 border-b border-border/50 flex flex-col md:flex-row justify-between md:items-center bg-card shrink-0 gap-4">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-rose-500 to-red-600 text-white flex items-center justify-center shadow-lg shadow-rose-500/20 shrink-0">
          ...
          {loading ? (
            ...
            <div className="space-y-6 md:space-y-8 max-w-[1400px] mx-auto pb-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                <div className="bg-card border border-border/50 p-6 rounded-3xl shadow-sm relative overflow-hidden group hover:border-rose-500/50 transition-colors">
                  <div className="flex justify-between items-start mb-4">
                    <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Despesa Total</p>
                    <div className="p-2 bg-rose-500/10 rounded-lg text-rose-500"><DollarSign size={16} /></div>
                  </div>
                ...
                <div className="bg-card border border-border/50 p-6 rounded-3xl shadow-sm relative overflow-hidden group hover:border-purple-500/50 transition-colors">
                  <div className="flex justify-between items-start mb-4">
                    <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Maior Despesa</p>
              ...
              {/* Row 2: Main Area Chart & Despesa por Serviço */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={data.monthlyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        ...
                        <Tooltip 
                          ...
                          formatter={(value: any) => [formatCurrency(value as number), 'Despesa']}
                          labelStyle={{ color: 'hsl(var(--foreground))', fontWeight: 'bold', marginBottom: '8px' }}
                        />
                        <Area type="monotone" dataKey="despesa" stroke="#10b981" strokeWidth={3} fillOpacity={1} fill="url(#colorDespesa)" activeDot={{ r: 6, strokeWidth: 0, fill: '#10b981' }} />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                ...
                <div className="lg:col-span-1 bg-card border border-border/50 p-6 md:p-8 rounded-3xl shadow-sm flex flex-col">
                  <h4 className="text-base font-bold text-foreground mb-6">Despesa por Serviço</h4>
                  <div className="h-[350px] w-full flex-1">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                      ...
                      {groupedOrders.map((group) => (
                        <React.Fragment key={group.key}>
                          ...
                          >
                            ...
                            <td className="px-6 py-4 font-bold text-rose-600">{formatCurrency(group.total)}</td>
                            <td className="px-6 py-4"></td>
                          </tr>
                          ...
                          {expandedMonths[group.key] && group.items.map((row) => (
                            <tr key={row.id} className="hover:bg-muted/10 transition-colors bg-background">
                              <td className="px-6 py-3 pl-14 font-medium text-foreground border-l-2 border-rose-500/30">{row.numero}</td>
                              ...
                              <td className="px-6 py-3 font-semibold text-rose-600">{formatCurrency(row.valor)}</td>
                              <td className="px-6 py-3">
                                <span className="bg-rose-500/10 text-rose-600 px-2.5 py-1 rounded-full text-xs font-semibold">Pago</span>
                              </td>
                            </tr>
                          ))}
                        </React.Fragment>
