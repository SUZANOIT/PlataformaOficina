...
export const QuotePdfTemplate = forwardRef<HTMLDivElement, QuotePdfTemplateProps>(({ data, company, workshops }, ref) => {
  ...
  return (
    <div
      ...
      style={{ fontFamily: isCurio ? "'Georgia', 'Times New Roman', serif" : "'Inter', 'Arial', sans-serif" }}
    >
      <style>{`
      ...
      <div>
        {/* Cabeçalho Premium */}
        <div className={`flex ${isCurio ? 'flex-row-reverse' : 'flex-row'} justify-between items-start border-b-2 ${isCurio ? 'border-indigo-900' : 'border-slate-900'} pb-6 mb-8`}>
          <div className={isCurio ? 'text-right' : 'text-left'}>
          ...
          <div className="flex gap-4 items-center">
            ...
            <div className={isCurio ? 'text-left' : 'text-right'}>
        ...
        {(hasClientData || hasVehicleData) && (
          <div 
            ...
            style={{ 
              display: 'flex', 
              flexDirection: isCurio ? 'column' : 'row', 
              gap: isCurio ? '16px' : '20px', 
              width: '100%' 
            }}
          >
            {/* Dados do Cliente */}
            {hasClientData && (
              <div 
                ...
                style={{ flex: 1, width: isCurio ? '100%' : (hasVehicleData ? '48%' : '100%') }}
              >
                <div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px 15px' }} className="text-[12px]">
                    ...
                    {data.client?.empresa && (
                      <div style={{ width: isCurio ? '45%' : (hasVehicleData ? '100%' : '45%'), minWidth: '120px' }}>
                    ...
                    {data.client?.email && (
                      <div style={{ width: isCurio ? '45%' : '100%', minWidth: '180px' }}>
                    ...
                    {data.client?.telefone && (
                      <div style={{ width: isCurio ? '45%' : '100%', minWidth: '120px' }}>
            ...
            {hasVehicleData && (
              <div 
                className={`${isCurio ? 'bg-indigo-50/20 border border-indigo-100' : 'bg-slate-50 border border-slate-200/80'} p-4 rounded-lg`}
                style={{ flex: 1, width: isCurio ? '100%' : (hasClientData ? '48%' : '100%') }}
              >
                <div>
                  <h3 className={`text-[11px] font-bold uppercase tracking-wider ${isCurio ? 'text-indigo-900 bg-indigo-900/5 px-2.5 py-1.5 rounded-md mb-3' : 'text-slate-500 border-b border-slate-200/80 pb-2 mb-3'}`}>
                    Dados do Veículo
        ...
        <div className="mb-8 avoid-page-break">
          ...
          {oficina && (
            <div className="mt-6 avoid-page-break">
              ...
              <div className="grid grid-cols-2 gap-y-2 gap-x-8 text-[12px] text-slate-600">
                <div>
                  <span className={`font-semibold ${isCurio ? 'text-indigo-950' : 'text-slate-800'}`}>Oficina Responsável:</span>{' '}
                  <span className="font-medium text-slate-900">{oficina.nome}</span>
                </div>
                {oficina.cnpj && (
                  <div>
                    <span className={`font-semibold ${isCurio ? 'text-indigo-950' : 'text-slate-800'}`}>CNPJ:</span>{' '}
                    <span className="font-medium text-slate-900">{oficina.cnpj}</span>
                  </div>
                )}
                {oficina.banco && (
                  <div>
                    <span className={`font-semibold ${isCurio ? 'text-indigo-950' : 'text-slate-800'}`}>Banco / Ag / Conta:</span>{' '}
                    <span className="font-medium text-slate-900">
                ...
                {oficina.chavePix && (
                  <div>
                    <span className={`font-semibold ${isCurio ? 'text-indigo-950' : 'text-slate-800'}`}>Chave PIX:</span>{' '}
                    <span className="font-medium text-slate-900">{oficina.chavePix}</span>
                  </div>
                )}
                {oficina.favorecido && (
                  <div className="col-span-2">
                    <span className={`font-semibold ${isCurio ? 'text-indigo-950' : 'text-slate-800'}`}>Favorecido:</span>{' '}
                    <span className="font-medium text-slate-900">
